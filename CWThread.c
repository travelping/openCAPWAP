...
void CWHandleTimer(CWTimerArg arg)
{
	...
	CWDebugLog("Timer Expired, Sent Signal(%d) to Thread: %ld", signalToRaise, (unsigned long int)requestedThreadPtr);
...
CWBool CWTimerRequest(int sec, CWThread * threadPtr, CWTimerID * idPtr, int signalToRaise)
{
	...
	CWDebugLog("Timer Request: thread(%ld), signal(%d)", (unsigned long int)*(arg->requestedThreadPtr), arg->signalToRaise);
